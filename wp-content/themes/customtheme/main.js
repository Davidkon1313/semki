...
// Select all section items
const sectionItems = document.querySelectorAll('.section__itemResponsive');

// Loop through each item
sectionItems.forEach((sectionItem) => {
  const switcher = sectionItem.querySelector('#switcher');
  const switcher2 = sectionItem.querySelector('#switcher2');

  if (switcher) {
    switcher.addEventListener('click', function () {
      sectionItem.classList.toggle('listed');
    });
  }
...
// Automatically click the first switcher2 element on page load
// Automatically click switcher2 in all section blocks with specific classes
document.addEventListener('DOMContentLoaded', function () {
  const sections = document.querySelectorAll('.S2, .S3, .S4, .S5'); // Select sections with these classes

  sections.forEach(section => {
    const switcher2 = section.querySelector('#switcher2'); // Select switcher2 by ID within each section
    if (switcher2) {
      switcher2.click(); // Trigger the click event
    }
  });
});
