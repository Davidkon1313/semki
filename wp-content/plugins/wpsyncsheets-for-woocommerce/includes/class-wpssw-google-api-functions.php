...
class WPSSW_Google_API_Functions extends \WPSSW_Google_API {
	...
	public function getClient( $flag = 0 ) {
		...
		$client->setScopes( array( 'https://www.googleapis.com/auth/userinfo.email', 'https://www.googleapis.com/auth/userinfo.profile', 'https://www.googleapis.com/auth/drive.readonly' ) );
	...
	public function createspreadsheetobject( $spreadsheetname = '' ) {
		$wpssw_requestbody = new \Google_Service_Sheets_Spreadsheet(
			array(
				'properties' => array(
					'title' => $spreadsheetname,
				),
	...
	public function insertdimensionrequests( $param = array(), $dimension = 'COLUMNS', $inheritstyle = false ) {
		...
		if ( $inheritstyle ) {
			...
			if ( 'ROWS' === (string) $dimension ) {
				$rowstartindex = $param['startindex'];
				$rowendindex   = $param['endindex'];
			} else {
				$columnstartindex = $param['startindex'];
				$columnendindex   = $param['endindex'];
			}
			...
			if ( 'ROWS' === (string) $dimension ) {
				...
				$requests = array_merge( $requests, $request1 );
			}
		}
	 ...
	 * Create Conditional Format Rule object.
	 *
	 * @param string $wpssw_spreadsheetid spreadsheetid.
	 * @param array  $batchupdaterequest batch update request array.
	 * @since 1.0.0
	 *
	 * @return object.
	 */
	public function addconditionalformatrule( $wpssw_spreadsheetid, $batchupdaterequest ) {
