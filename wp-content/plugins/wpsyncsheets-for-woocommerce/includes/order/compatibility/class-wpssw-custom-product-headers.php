...
if ( ! defined( 'ABSPATH' ) ) {
	exit; }
if ( ! class_exists( 'WPSSW_Custom_Product_Headers' ) ) :
	/**
	 * Class WPSSW_Custom_Product_Headers.
	 */
	class WPSSW_Custom_Product_Headers extends WPSSW_Order_Utils {
		/**
		 * Store header list.
		 *
		...
		public static function prepare_headers() {
			$wpssw_custom_headers = apply_filters( 'wpssw_custom_product_headers', array() ); // use this.
			if ( empty( $wpssw_custom_headers ) ) {
				$wpssw_custom_headers = array();
			}
		...
		public static function get_header_list( $headers = array() ) {
			...
			if ( ! empty( self::$wpssw_headers ) ) {
				$headers['WPSSW_Custom_Product_Headers'] = self::$wpssw_headers;
			}
			return $headers;
		}
		/**
		...
		public static function prepare_value( $wpssw_headers_name, $wpssw_order, $wpssw_operation, $wpssw_custom_value = array(), $wpssw_product_headers = array() ) {
			...
			$wpssw_value             = array();
			$wpssw_extra_headers_val = apply_filters( 'wpssw_custom_product_values', '', $wpssw_order->get_id(), $wpssw_headers_name ); // use this.
			...
			if ( $wpssw_header_type ) {
				...
				$wpssw_ordervalue[] = implode( ',', $wpssw_value );
				return $wpssw_ordervalue;
			}
		}
	}
	new WPSSW_Custom_Product_Headers();
endif;
