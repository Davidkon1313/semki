...
if ( ! defined( 'ABSPATH' ) ) {
	exit; }
if ( ! class_exists( 'WPSSW_WooCommerce_PDF' ) ) :
	/**
	 * Class WPSSW_WooCommerce_PDF.
	 */
	class WPSSW_WooCommerce_PDF extends WPSSW_Order_Utils {
		/**
		 * Store header list.
		 *
		...
		protected function prepare_headers() {
			self::$wpssw_headers = array( 'Invoice Number' );
		}
		/**
		 * Get Header List.
		 *
		...
		public static function get_header_list( $headers = array() ) {
			if ( ! empty( self::$wpssw_headers ) ) {
				$headers['WPSSW_WooCommerce_PDF'] = self::$wpssw_headers;
			}
			return $headers;
		}
		/**
		...
		public static function prepare_value( $wpssw_headers_name, $wpssw_order, $wpssw_operation, $wpssw_custom_value = array(), $wpssw_product_headers = array() ) {
			$wpssw_value = array();
			if ( 'Invoice Number' === (string) $wpssw_headers_name ) {
				$wpssw_value[] = $wpssw_order->get_meta( '_invoice_number_display' );
			}
			return $wpssw_value;
		}
	}
	new WPSSW_WooCommerce_PDF();
endif;
