...
if ( ! defined( 'ABSPATH' ) ) {
	exit; }
if ( ! class_exists( 'WPSSW_WC_Xero' ) ) :
	/**
	 * Class WPSSW_WC_Xero.
	 */
	class WPSSW_WC_Xero extends WPSSW_Order_Utils {
		/**
		 * Store header list.
		 *
		...
		protected function prepare_headers() {
			self::$wpssw_headers = array( 'Xero Invoice Id' );
		}
		/**
		 * Get Header List.
		 *
		...
		public static function get_header_list( $headers = array() ) {
			if ( ! empty( self::$wpssw_headers ) ) {
				$headers['WPSSW_WC_Xero'] = self::$wpssw_headers;
			}
			return $headers;
		}
		/**
		...
		public static function prepare_value( $wpssw_headers_name, $wpssw_order, $wpssw_operation, $wpssw_custom_value = array(), $wpssw_product_headers = array() ) {
			$wpssw_value = array();
			if ( 'Xero Invoice Id' === (string) $wpssw_headers_name ) {
				$wpssw_value[] = $wpssw_order->get_meta( '_xero_invoice_id' );
			}
			return $wpssw_value;
		}
	}
