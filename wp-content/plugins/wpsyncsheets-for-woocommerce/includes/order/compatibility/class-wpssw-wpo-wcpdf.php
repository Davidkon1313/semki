...
if ( ! defined( 'ABSPATH' ) ) {
	exit; }
if ( ! class_exists( 'WPSSW_WPO_WCPDF' ) ) :
	/**
	 * Class WPSSW_WPO_WCPDF.
	 */
	class WPSSW_WPO_WCPDF extends WPSSW_Order_Utils {
		/**
		 * Store header list.
		 *
		...
		public static function get_header_list( $headers = array() ) {
			if ( ! empty( self::$wpssw_headers ) ) {
				$headers['WPSSW_WPO_WCPDF'] = self::$wpssw_headers;
			}
			return $headers;
		}
		/**
		...
		public static function prepare_value( $wpssw_headers_name, $wpssw_order, $wpssw_operation, $wpssw_custom_value = array(), $wpssw_product_headers = array() ) {
			...
			if ( 'Invoice Number' === (string) $wpssw_headers_name ) {
				$wpssw_value[] = $wpssw_order->get_meta( '_wcpdf_invoice_number' );
			}
			return $wpssw_value;
		}
	}
