				...
				if ( 'insert' === (string) $wpsslw_operation ) {
					...
					if ( 'Billing Postcode' === (string) $wpsslw_headers_name ) {
						...
						if ( $wpsslw_order->get_billing_country() ) {
							$wpsslw_insert_val = WC()->countries->countries[ $wpsslw_order->get_billing_country() ];
						} else {
							$wpsslw_insert_val = '';
						}
...
' . $wpsslw_billing_country;
			...
			if ( 'Shipping' === (string) $wpsslw_arr[0] ) {
				...
				if ( 'Shipping Method Title' === (string) $wpsslw_headers_name ) {
					...
					$wpsslw_insert_val = isset( WC()->countries->countries[ $wpsslw_order->get_shipping_country() ] ) ? WC()->countries->countries[ $wpsslw_order->get_shipping_country() ] : '';
